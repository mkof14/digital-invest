...
const overviewTexts: Record<string, Record<string, string>> = {
  ...
  'saven': {
    en: 'SAVEN is not a wellness app — it\'s an Infrastructure of Continuous Execution. Built on a 5-step operational cycle (Context → Action → Verification → Control → Support), it sits within the 4-level BioMath architecture hierarchy. SAVEN integrates with various types of robots and physical execution devices, self-improves and learns from each cycle. It doesn\'t motivate — it creates an environment where execution happens automatically. The "Three Environments" logic separates physical, digital, and social contexts, and the economic model is built around high exit costs and growing LTV.',
    ru: 'SAVEN — не велнес-приложение, а Инфраструктура Непрерывного Исполнения. Построен на 5-шаговом операционном цикле (Контекст → Действие → Верификация → Контроль → Поддержка) внутри 4-уровневой архитектуры BioMath. SAVEN интегрируется с различными типами роботов и устройствами физического исполнения команд, самосовершенствуется и обучается с каждым циклом. Не мотивирует — создаёт среду, где исполнение происходит автоматически. Логика «Трёх Сред» разделяет физический, цифровой и социальный контексты, а экономическая модель построена на высоких затратах выхода и растущем LTV.',
    uk: 'SAVEN — не велнес-додаток, а Інфраструктура Безперервного Виконання. Побудований на 5-кроковому операційному циклі (Контекст → Дія → Верифікація → Контроль → Підтримка) всередині 4-рівневої архітектури BioMath. SAVEN інтегрується з різними типами роботів та пристроями фізичного виконання команд, самовдосконалюється та навчається з кожним циклом. Не мотивує — створює середовище, де виконання відбувається автоматично. Логіка «Трьох Середовищ» розділяє фізичний, цифровий та соціальний контексти.',
    fr: 'SAVEN n\'est pas une appli bien-être — c\'est une Infrastructure d\'Exécution Continue. Basé sur un cycle en 5 étapes au sein de l\'architecture BioMath à 4 niveaux. SAVEN s\'intègre à différents types de robots et dispositifs d\'exécution physique, s\'auto-améliore et apprend à chaque cycle. Il ne motive pas — il crée un environnement où l\'exécution est automatique. La logique des « Trois Environnements » sépare les contextes physique, numérique et social.',
    ar: 'SAVEN ليس تطبيق صحة — إنه بنية التنفيذ المستمر. مبني على دورة من 5 خطوات ضمن هرمية BioMath من 4 مستويات. يتكامل SAVEN مع أنواع مختلفة من الروبوتات وأجهزة التنفيذ المادي للأوامر، ويتحسن ذاتياً ويتعلم مع كل دورة. لا يحفز — بل يخلق بيئة يحدث فيها التنفيذ تلقائياً. منطق "البيئات الثلاث" يفصل السياقات المادية والرقمية والاجتماعية.',
    ja: 'SAVENはウェルネスアプリではなく、継続的実行のインフラです。4レベルのBioMathアーキテクチャ内の5ステップ運用サイクルで構築。様々な種類のロボットや物理的コマンド実行デバイスと統合し、各サイクルで自己改善・学習します。動機づけではなく、実行が自動的に起こる環境を作ります。「三つの環境」ロジックが物理的・デジタル・社会的コンテキストを分離します。',
    he: 'SAVEN הוא לא אפליקציית בריאות — הוא תשתית של ביצוע רציף. בנוי על מחזור בן 5 שלבים בתוך הירארכיית BioMath בת 4 רמות. SAVEN משתלב עם סוגים שונים של רובוטים ומכשירי ביצוע פיזי של פקודות, משתפר עצמית ולומד עם כל מחזור. לא מניע — יוצר סביבה שבה הביצוע קורה אוטומטית. לוגיקת "שלוש הסביבות" מפרידה הקשרים פיזיים, דיגיטליים וחברתיים.'
  },
  'luna-balance': {
